...
- Build a SvelteKit frontend (design system in `docs/design/design.md` + `wireframe.md`: Cabin/Inconsolata typography, OKLCH color tokens, 3-column chat/pipeline/output layout) with a Svelte Flow orchestrator-hub node diagram (Orchestrator + 3 agents + HITL gate + MCP Tools node, numbered dispatch edges) showing agents lighting up sequentially, tool calls expanding inline as collapsible rows inside agent node cards, a chat panel (entry prompt, conversation thread, scenario quick-buttons, chat input), structured output panel
- Render Langfuse trace data as a flat observation table (one row per observation: name, type, latency, tokens, cost, level) in a trace panel below the diagram
...
- `pipeline-ui`: SvelteKit frontend (design system: `docs/design/design.md` + `wireframe.md`) — Svelte Flow orchestrator-hub node diagram (Orchestrator + 3 agents + HITL gate + MCP Tools node, numbered dispatch edges) with live agent states (idle → running → done, plus warning/paused) and per-agent step progress + inline tool-call rows, full-width flat trace summary, tabbed chat panel (entry prompt, conversation thread carrying agent responses, scenario quick-buttons, chat input, results tab that the panel auto-switches to on completion)
- `trace-panel`: Flat Langfuse observation table (name, type, latency, tokens, cost, level — no per-agent grouping) with pipeline aggregate footer, fetched via Langfuse API
