...
- `pipeline-ui`: SvelteKit frontend (design system: `docs/design/design.md` + `wireframe.md`) — Svelte Flow node diagram with live agent states (idle → running → done, plus warning/paused) rendered as compact nodes, full-width trace summary, tabbed chat panel (conversation thread, scenario quick-buttons, chat input, results tab), routing strategy toggle
