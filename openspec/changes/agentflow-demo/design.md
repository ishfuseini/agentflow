...
- **Chat-based interaction (Col 1):** Scenarios are quick buttons that send a pre-built message into a conversation thread. User/system chat bubbles (right-aligned purple / left-aligned grey). Single-row input + send button (Enter submits, Shift+Enter newline). "← New conversation" reset after a run.
...
- **Tool calls:** Collapsible rows *inside* agent NodeCards (not separate tool-call nodes, not trace-panel child rows). Each shows Zap icon + `name()` + ChevronDown; expand reveals result text with `→` prefix.
...
- **TracePanel:** Per-agent progress row (status icon · label · latency · cost · ✓/⚠) + footer totals (time · cost · eval). No tool-call child rows.
