...
- [x] 3.1 Register the agentflow-mcp server as a tool provider at `https://agentflow-mcp.fly.dev/mcp` (HTTP stream transport for remote; stdio for local dev via MCP Inspector). Verify all 7 MCP tools (`brand_search`, `brand_context_lookup`, `arch_pattern_lookup`, `arch_pattern_references`, `arch_diagram`, `tool_selection_lookup`, `risk_policy_lookup`) are discoverable via `scripts/mcp-list-check.ts` or MCP Inspector
- [x] 3.2 Wire brand resolution: call `brand_search` with `query` (company name or prefix from the prompt/scenario) and optional `strategy`. Verify the tool returns candidates with `name`, `domain`, and `logo_url`, and the selected candidate's logo is shown in the conversation immediately, before `brand_context_lookup` runs
- [x] 3.3 Wire `brand_context_lookup` with the `domain` from the confirmed `brand_search` candidate. Verify the tool returns `company_name`, `domain`, `industry_hint`, `description`, `tags`, `positioning` (`value_proposition`, `target_audience`, `products_and_services`), `brand` (`voice`, `style`), `logo_url`, and `confidence`
- [x] 3.4 Wire the Architect Agent to call `arch_pattern_lookup` with `industry`, `data_stack`, `constraints`, and optionally `cloud`/`latency` (derived from Qualifier output). Verify the tool returns `pattern_id`, `architecture_summary`, `recommended_components`, `data_zones`, `integration_notes`, and `confidence` — with no inline `diagram_data` or `source_references`
- [x] 3.5 Wire the Architect Agent to call `arch_pattern_references` with `pattern_id` plus the original `industry`, `data_stack`, and `constraints`. Verify the tool returns `source_references` (path, title, source_url) and that they accompany the pattern when presented for confirmation
- [x] 3.6 Wire the Architect Agent to call `tool_selection_lookup` with `use_case`, `data_stack`, `constraints`, and optionally `latency`. Verify the tool returns `recommended_platform`, `cloud_fit`, `reasoning`, and `alternatives`
- [x] 3.7 Wire the Risk Checker Agent to call `risk_policy_lookup` with `industry`, `data_classification`, `region`, `deployment`, and optionally `constraints`. Verify the tool returns `required_controls`, `risk_flags`, `hitl_required`, and `review_reason`, and that the call completes before any `arch_diagram` fetch
- [x] 3.8 Wire on-demand diagram fetch: call `arch_diagram` with `pattern_id` only when a diagram is requested, after risk evaluation has completed. Verify `available=true` returns `diagram_data` (components, connections, boundaries), and the fallback pattern returns `available=false` with an explanatory message so no diagram renders
- [x] 3.9 Implement graceful fallback for `brand_context_lookup` unavailable: pipeline continues without brand context, architecture diagram renders without branded header. Verify the pipeline completes successfully when `brand_context_lookup` returns unavailable
- [x] 3.10 Implement graceful fallback for low-confidence `arch_pattern_lookup` match (`generic_enterprise_ai_poc`, confidence < 0.5): Architect Agent flags weak match, no diagram is fetched or rendered, pipeline continues. Verify a custom prompt that doesn't match any curated pattern completes with a weak-match flag and no diagram
- [x] 3.11 Verify end-to-end MCP integration: run the Healthcare scenario and confirm the tool calls fire in order (brand_search → brand_context_lookup → arch_pattern_lookup → arch_pattern_references → tool_selection_lookup → risk_policy_lookup, then arch_diagram only if requested) with correct data flowing between agents
